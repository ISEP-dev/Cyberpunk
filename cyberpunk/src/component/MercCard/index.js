...
const MercCard = ({ merc }) => {
    return (
        <div className={`${!merc.isAlive ? "bg-gray-400 opacity-90" : "bg-gray-800"} z-0 relative flex flex-col shadow-lg overflow-hidden sm:rounded-md p-6 m-4 w-1/5 text-white`}>
            <i className={`fas fa-${merc.isAlive ? "heart text-red-600" : "heart-broken text-gray-300"} absolute right-4 top-4 float-right`}/>
            <span className="text-center font-bold text-xl truncate">{merc.nickname}</span>
